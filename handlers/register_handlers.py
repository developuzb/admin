...
def create_invoice_image(order, amount):
    ...
    try:
        ...
        font_small = ImageFont.truetype(
            "cour.ttf", 14)  # Kattaroq pastki yozuv
    except IOError:
    ...
    draw.text(((width - header_width) // 2, y),
              header, font=font_title, fill='#000000')
    y += 40
    ...
    draw.text(((width - title_width) // 2, y),
              title, font=font_title, fill='#000000')
    y += 50
    ...
    y += 30
    draw.text((50, y), "Karta: 8600 3104 7319 9081",
    ...
    draw.text(((width - footer_width) // 2, y),
              footer, font=font_small, fill='#000000')
    y += 20
    ...
    draw.text(((width - contact_width) // 2, y),
              contact, font=font_small, fill='#000000')
    y += 30
    ...
    draw.text(((width - qr_label_width) // 2, y),
              qr_label, font=font_small, fill='#000000')
...
def create_receipt_image(order, amount, confirmation_time):
    ...
    try:
        ...
        font_stamp = ImageFont.truetype(
            "cour.ttf", 18)  # Kichikroq muhr shrifti
    except IOError:
        font_title = ImageFont.load_default()
        font_body = ImageFont.load_default()
        font_small = ImageFont.load_default()
    ...
    stamp_draw.rectangle((10, 10, 140, 90), outline='#000080',
                         width=2)  # To‘rtburchak, qalin chet
    ...
    # Soya effekti
    stamp_draw.text((30 + 2, 40 + 2), stamp_text,
                    font=font_stamp, fill=(0, 0, 128, 100))
    # Asosiy matn
    stamp_draw.text((30, 40), stamp_text, font=font_stamp, fill='#000080')
    # Engil burish
    stamp_img = stamp_img.rotate(
        5, expand=True, resample=Image.Resampling.BICUBIC)
...
try:
    ...
    logger.critical(
        f".env faylida xato: ADMIN_ID yoki GROUP_ID noto'g'ri. {e}")
...
(ASK_ACTION, ASK_DELETE_ID, ADD_SERVICE_ID, ADD_SERVICE_NAME, ADD_SERVICE_PRICE,
 ...
 # SETTINGS_ADMIN qo'shildi
 SEARCH_SERVICE, TOGGLE_VISIBILITY_ID, GROUP_BY_CATEGORY, SETTINGS_ADMIN) = range(25)
# Database setup
...
def register_handlers(dp: Dispatcher):
    ...
    # Xizmat tanlash: #XIZMAT#id
    dp.message.register(trigger_inline_handler, F.text.regexp(r"#XIZMAT#\d+"))

    # Roziman tugmasi
    dp.callback_query.register(roziman_handler, F.data == "confirm_service")
...
# ✅ Client-facing Telegram Bot Functions (FULL extraction)

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    user_id_str = str(user_id)
    args = context.args  # <== args orqali deep-linkni olamiz
    ...
    # ✅ 000xxx deep-link borligini tekshiramiz
    if args and re.match(r'^000[0-9]{3}$', args[0]):
        return await handle_start_with_service(update, context, args[0])

    logger.info(f"/start buyrug‘i keldi. Foydalanuvchi ID: {user_id}")
    now_str = datetime.now(pytz.timezone('Asia/Tashkent')
    ...
    if not user_data:
        ...
        markup = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton(
                "🆘 Yordam kerak", callback_data="help_request")]
        ])
        await update.message.reply_text(
    ...
    if len(user_data.get("orders", [])) >= 3 and not user_data.get("is_loyal"):
    ...
    # 💬 Matnlar A/B guruh va sodiqlik asosida
    if is_loyal:
        quotes = [
    ...
    markup_buttons = [
        ...
        [InlineKeyboardButton("📜 Buyurtmalar tarixi",
                              callback_data="show_history")],
        [InlineKeyboardButton("🎁 Sodiqlik dasturi",
                              callback_data="bonus_services")]
    ]
    if is_loyal:
        markup_buttons.append([InlineKeyboardButton(
            " Mening bonusim 🎁 ", callback_data="bonus_services")])
...
async def handle_start_with_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    message = update.message
    match = re.match(r"/start (000\d{3})", message.text)
    if not match:
        await message.reply_text("❌ Xizmat ID noto‘g‘ri. Havolani tekshiring.")
        return
    ...
    if not service:
        await message.reply_text("❌ Bunday xizmat topilmadi.")
        return

    name, description, price = service
    cashback = round(int(price) * 0.1)
    rating = "⭐ 4.8/5"
    fomo = "✅ *Hozir tanlasangiz, 10% chegirma!*"

    text = (
        f"🖨️ *Zo‘r tanlov, {update.effective_user.first_name or 'mijoz'}!*\n"
        f"Xizmat: *{name}*\n"
        f"Narxi: *{price} so‘m*\n"
        f"🎁 Keshbek: *{cashback} so‘m*\n"
        f"{rating} | {fomo}"
    )

    buttons = [
        [InlineKeyboardButton(
            "✅ Tez buyurtma", callback_data=f"order_{service_id}")],
        [InlineKeyboardButton("📎 Fayl yuborish", callback_data="send_file")],
        [InlineKeyboardButton("⬅️ Orqaga", callback_data="go_back")],
        [InlineKeyboardButton("✏️ Javobimni o‘zgartirish",
                              callback_data="change_response")]
    ]
...
async def add_service_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id != ADMIN_ID:
        ...
        logger.warning(
            f"❌ Noto‘g‘ri admin kirish urinishi: user_id={update.effective_user.id}")
        return ConversationHandler.END
    ...
    logger.info(
        f"✅ Yangi xizmat qo‘shish boshlandi: user_id={query.from_user.id}")
...
async def settings_admin_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(
        f"✅ Yangi admin tayinlash boshlandi: user_id={query.from_user.id}")
    ...
    try:
        ...
        try:
            ...
            await update.message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton(
                        "⬅️ Orqaga", callback_data="admin_settings")]
                ])
            )
            return SETTINGS_ADMIN
        ...
        await update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(
                    "⬅️ Sozlamalarga", callback_data="admin_settings")]
            ])
        )
        logger.info(
            f"✅ Yangi admin tayinlandi: user_id={update.effective_user.id}, new_admin_id={new_admin_id}")
        return ConversationHandler.END
    except ValueError:
        await update.message.reply_text(
        ...
        logger.error(
            f"❌ Noto‘g‘ri ID formati: user_id={update.effective_user.id}, input={update.message.text}")
        return SETTINGS_ADMIN
...
async def get_service_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if any(s['id'] == service_id for s in services):
            ...
            logger.warning(
                f"❌ Dublikat ID kiritildi: user_id={update.effective_user.id}, id={service_id}")
            return ADD_SERVICE_ID
        context.user_data['id'] = service_id
        await update.message.reply_text("2/6: Xizmat nomini kiriting:")
        logger.info(
            f"✅ Xizmat ID kiritildi: user_id={update.effective_user.id}, id={service_id}")
...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(
        f"✅ Xizmat nomi kiritildi: user_id={update.effective_user.id}, name={name}")
...
async def get_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.info(
            f"✅ Xizmat narxi kiritildi: user_id={update.effective_user.id}, price={price}")
        return ADD_SERVICE_PAYMENTS
    except ValueError:
        await update.message.reply_text("❌ Faqat raqam kiriting (masalan, 10000). Qayta urining:")
        logger.error(
            f"❌ Noto‘g‘ri narx formati: user_id={update.effective_user.id}, input={update.message.text}")
        return ADD_SERVICE_PRICE
...
async def get_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.lower() == "tugatish":
        ...
        logger.info(
            f"✅ To‘lov usullari tanlandi: user_id={update.effective_user.id}, methods={context.user_data['payment_methods']}")
        return ADD_SERVICE_IMAGE
    context.user_data['payment_methods'].append(text)
    logger.info(
        f"✅ To‘lov usuli qo‘shildi: user_id={update.effective_user.id}, method={text}")
    return ADD_SERVICE_PAYMENTS
...
async def get_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text and update.message.text.lower() in ["yo‘q", "yoq", "rasm yo‘q"]:
        ...
        logger.info(
            f"✅ Rasm yo‘q deb tanlandi: user_id={update.effective_user.id}")
    elif update.message.photo:
        context.user_data['image'] = update.message.photo[-1].file_id
        logger.info(
            f"✅ Rasm kiritildi: user_id={update.effective_user.id}, file_id={context.user_data['image']}")
    else:
...
async def get_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("⬅️ Xizmatlar menyusiga",
                                  callback_data="admin_services")]
        ])
    )
    logger.info(
        f"✅ Yangi xizmat saqlandi: user_id={update.effective_user.id}, id={new_service['id']}, name={new_service['name']}")
...
async def trigger_inline_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if text.strip().lower() in ("/info", "info"):
            btn = InlineKeyboardMarkup([
                [InlineKeyboardButton(
                    "📲 Obuna bo‘lish", url="https://t.me/texnosetUZ")]
            ])
    ...
    if 'Click' in service['payment_methods']:
        ...
        buttons.insert(0, [InlineKeyboardButton(
            "💳 Click orqali to‘lash", url=click_url)])
    ...
    if update.message and update.message.text == "🚀 Xizmat izlash boshlandi...":
        try:
            await update.message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton(
                        "📢 Kanalimizga obuna bo‘ling!", url="https://t.me/texnosetUZ")]
                ])
            )
        except Exception as e:
...
async def fallback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    name = context.bot_data.get(f"user_{user_id}", {}).get(
        "name", "Hurmatli mijoz")
    ...
    markup = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🆘 Yordam so‘rash",
                              callback_data="help_request")]
    ])
...
async def continue_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "waiting_for_phone":
        ...
        markup = InlineKeyboardMarkup([
            [InlineKeyboardButton("📱 Shu bot orqali",
                                  callback_data="contact_bot")],
            [InlineKeyboardButton("☎️ Qo‘ng‘iroq orqali",
                                  callback_data="contact_call")],
            [InlineKeyboardButton(
                "📩 SMS orqali", callback_data="contact_sms")],
            [InlineKeyboardButton(
                "🔄 Boshqa usul", callback_data="contact_other")]
        ])
    ...
    logger.info(
        f"Davom etish xabari yuborildi: user_id={user_id}, step={step}")
    return ConversationHandler.END
...
async def help_request_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_data and user_data.get("thread_id") and user_data.get("is_operator_started"):
        ...
        logger.info(
            f"Yordam so‘rovi: user_id={user_id}, mavjud thread_id={thread_id}")
    ...
    try:
        ...
        save_bot_data(context)
        logger.info(
            f"Yordam so‘rovi guruhga yuborildi: user_id={user_id}, thread_id={topic.message_thread_id}")
        ...
        await query.message.edit_text(
            ...
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔁 Qayta boshlash",
                                      callback_data="restart")]
            ])
        )
        logger.error(
            f"Yordam so‘rovi yuborishda xato: user_id={user_id}, xato={e}")
...
async def phone_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    step = context.user_data.get('step')
    if step != 'waiting_for_phone':
        logger.warning("📵 phone_handler: noto‘g‘ri step")
        return

    phone = None
    if update.message.contact:
        phone = update.message.contact.phone_number
        logger.info(f"📲 Kontakt orqali: {phone}")
    elif update.message.text:
        cleaned = re.sub(r'\D', '', update.message.text.strip())
        logger.info(f"🧼 Matndan tozalandi: {cleaned}")
        if 9 <= len(cleaned) <= 13:
            phone = cleaned
        else:
            await update.message.reply_text(
    ...
    markup = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔄 Boshqa usul orqali",
...
async def contact_time_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not all([service, phone, contact_method, order_id, user_id]):
        logger.error(
            f"❌ Ma'lumotlar to‘liq emas: service={service}, phone={phone}, contact_method={contact_method}, order_id={order_id}, user_id={user_id}")
    ...
    try:
        ...
        logger.info(
            f"✅ Buyurtma yuborildi: order_id={order_id}, thread_id={topic.message_thread_id}")
        context.bot_data[f"user_{user_id}"] = {
            'order_id': order_id,
            'thread_id': topic.message_thread_id,
            'is_operator_started': False
        ...
        logger.info(
            f"✅ contact_time_handler: context.bot_data yangilandi: user_id={user_id}, data={context.bot_data[f'user_{user_id}']}")
    except Exception as e:
        logger.error(
            f"⚠️ Buyurtma yuborishda xato: user_id={user_id}, xato={e}")
...
async def handle_help_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("step") != "waiting_for_help_question":
        logger.warning(
            f"handle_help_question: noto‘g‘ri step, joriy step: {context.user_data.get('step')}")
        return

    user_id = str(update.effective_user.id)
    name = USERS.get(user_id, {}).get(
        'name', update.effective_user.full_name or "Foydalanuvchi")
    ...
    if f"user_{user_id}" in context.bot_data and context.bot_data[f"user_{user_id}"].get("help_question"):
        ...
        logger.info(
            f"✅ Eski yordam so‘rovi tozalandi: user_id={user_id}, thread_id={old_thread_id}")
    ...
    try:
        ...
        logger.error(
            f"❌ Yordam so‘rovini guruhga yuborishda xato: user_id={user_id}, xato={e}")
...
async def send_order_to_group(context, order_id, service, phone, contact_method, contact_time, text, thread_id, user_id):
    ...
    try:
        ...
        logger.info(
            f"✅ Guruhga xabar yuborildi: message_id={msg.message_id}, user_id={user_id}, thread_id={thread_id}")
        ...
        logger.info(
            f"✅ send_order_to_group: context.bot_data yangilandi: user_id={user_id}, data={context.bot_data[f'user_{user_id}']}")
    except Exception as e:
        logger.error(
            f"❌ Guruhga buyurtma yuborishda xato: user_id={user_id}, thread_id={thread_id}, xato={e}")
        raise
...
async def send_rating_request(user_id: int, identifier: str, context: ContextTypes.DEFAULT_TYPE, is_help_request: bool = False):
    try:
        rating_keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton(
                    "1 😞", callback_data=f"rate_{identifier}_1"),
                InlineKeyboardButton(
                    "2 😕", callback_data=f"rate_{identifier}_2"),
                InlineKeyboardButton(
                    "3 😐", callback_data=f"rate_{identifier}_3"),
                InlineKeyboardButton(
                    "4 🙂", callback_data=f"rate_{identifier}_4"),
                InlineKeyboardButton(
                    "5 🤩", callback_data=f"rate_{identifier}_5"),
            ]
        ])
        ...
        logger.info(
            f"✅ Baholash so‘rovi yuborildi: user_id={user_id}, identifier={identifier}, is_help_request={is_help_request}")

    except telegram.error.BadRequest as e:
        if "chat not found" in str(e).lower():
            logger.error(
                f"❌ Baholash so‘rovi yuborilmadi — chat topilmadi: user_id={user_id}")
        else:
            logger.error(
                f"❌ Baholash so‘rovi yuborishda BadRequest: user_id={user_id}, error={e}")
    except Exception as e:
        logger.error(
            f"❌ Baholash so‘rovi yuborishda umumiy xato: user_id={user_id}, error={e}")
...
async def group_order_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not info:
        logger.error(
            f"❌ Buyurtma ma'lumotlari topilmadi, message_id={msg_id}, bot_data={context.bot_data}")
    ...
    data = query.data
    logger.info(
        f"✅ Guruh tugmasi bosildi: order_id={order_id}, data={data}, user_id={user_id}")

    try:
        await query.edit_message_reply_markup(reply_markup=None)
    except Exception as e:
        logger.error(
            f"❌ Tugmalarni o‘chirishda xato: message_id={msg_id}, xato={e}")
    ...
    if data.startswith("group_accept_"):
        ...
        if user_id:
            try:
                ...
                if f"user_{user_id}" in context.bot_data:
                    ...
                    logger.info(
                        f"✅ Operator suhbatni boshladi: user_id={user_id}, bot_data={context.bot_data[f'user_{user_id}']}")
                    save_bot_data(context)  # Qo‘shish
            except telegram.error.BadRequest as e:
                logger.error(
                    f"❌ Foydalanuvchiga xabar yuborishda xato: user_id={user_id}, xato={e}")
        ...
        if user_id:
            try:
                ...
                logger.error(
                    f"❌ Foydalanuvchiga bekor qilish xabari yuborishda xato: user_id={user_id}, xato={e}")
...
async def accept_help_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id_str = str(user_id)
    name = USERS.get(user_id_str, {}).get(
        'name', query.from_user.full_name or "Foydalanuvchi")
    ...
    try:
        ...
        logger.error(
            f"❌ Operator ulanayotganda mijozga xabar yuborilmadi: user_id={user_id}, xato={e}")
        await query.message.reply_text(f"❌ Foydalanuvchiga xabar yuborib bo‘lmadi: {e}")
...
async def user_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = update.message
    name = USERS.get(user_id, {}).get(
        'name') or update.effective_user.full_name or "Mijoz"

    info = context.bot_data.get(f"user_{user_id}")
    if not info or "thread_id" not in info:
        logger.error(
            f"❌ Mijoz topilmadi yoki thread_id yo‘q: user_id={user_id}, bot_data={context.bot_data}")
    ...
    if not is_operator_started:
        logger.info(
            f"⏳ Operator hali yozmagan: user_id={user_id}, thread_id={thread_id}")
        await message.reply_text(
    ...
    try:
        ...
        logger.error(
            f"❌ Guruhni tekshirishda xato: GROUP_ID={GROUP_ID}, xato={e}")
    ...
    try:
        ...
        logger.info(
            f"✅ Mijozdan xabar yuborildi: user_id={user_id}, thread_id={thread_id}, bot_data={context.bot_data.get(f'user_{user_id}')}")
    except Exception as e:
        logger.error(
            f"❌ Mijozdan xabarni yuborishda xato: user_id={user_id}, thread_id={thread_id}, xato={e}")
...
async def relay_from_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message.text and message.text.startswith(("/bajarildi", "/bekor")):
        logger.info(
            f"🔄 Buyruq aniqlangan: {message.text}, relay_from_group bajarilmadi.")
        return
    ...
    if not info:
        logger.error(
            f"❌ user_id topilmadi: thread_id={thread_id}, bot_data={context.bot_data}")
        await context.bot.send_message(
            chat_id=GROUP_ID,
            message_thread_id=thread_id,
    ...
    try:
        ...
        if chat.type != "private":
            logger.error(
                f"❌ Xatolik: user_id={user_id} xususiy chat emas, turi: {chat.type}")
            await context.bot.send_message(
                chat_id=GROUP_ID,
                message_thread_id=thread_id,
        ...
        logger.error(
            f"❌ Chatni olishda xatolik: user_id={user_id}, xato={e}, bot_data={context.bot_data.get(f'user_{user_id}')}")
        await context.bot.send_message(
            chat_id=GROUP_ID,
            message_thread_id=thread_id,
    ...
    if f"user_{user_id}" in context.bot_data:
        context.bot_data[f"user_{user_id}"]['is_operator_started'] = True
        logger.info(
            f"✅ Operator suhbatni boshladi: user_id={user_id}, thread_id={thread_id}, bot_data={context.bot_data[f'user_{user_id}']}")
    ...
    try:
        ...
        logger.error(
            f"❌ relay_from_group: Xabarni yuborishda xato: user_id={user_id}, xato={e}")
        await context.bot.send_message(
            chat_id=GROUP_ID,
            message_thread_id=thread_id,
...
async def universal_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(
        f"universal_router: user_id={user_id}, step={step}, message={message}")
    ...
    if not user_data or 'thread_id' not in user_data:
        await update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(
                    "📩 Buyurtma berish", switch_inline_query_current_chat="")]
            ])
        )
        logger.warning(
            f"❌ thread_id topilmadi yoki user_data yo‘q: user_id={user_id}")
        return
    ...
    user_name = USERS.get(str(user_id), {}).get(
    ...
    if message:
        ...
        if len(message) > 4096:
            ...
            logger.warning(
                f"✅ Matn uzunligi qisqartirildi: user_id={user_id}, uzunlik={len(message)}")
        ...
        try:
            ...
            logger.info(
                f"📤 Matnli xabar yuborildi: user_id={user_id}, thread_id={thread_id}")
        except Exception as e:
            await update.message.reply_text("❌ Xabar yuborishda xato yuz berdi. Qayta urinib ko‘ring.")
            logger.error(
                f"❌ Matnli xabar yuborishda xato: user_id={user_id}, thread_id={thread_id}, xato={e}")
        ...
        try:
            ...
            logger.info(
                f"📤 Media xabar yuborildi: user_id={user_id}, thread_id={thread_id}")
        except Exception as e:
            await update.message.reply_text("❌ Fayl yuborishda xato yuz berdi. Qayta urinib ko‘ring.")
            logger.error(
                f"❌ Media yuborishda xato: user_id={user_id}, thread_id={thread_id}, xato={e}")
    else:
        await update.message.reply_text("❌ Iltimos, matnli xabar yuboring yoki fayl yuklang.")
        logger.warning(
            f"❌ To‘g‘ri formatdagi xabar topilmadi: user_id={user_id}")
...
async def rating_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # USERS ni yangilash
    if user_id_str not in USERS:
        USERS[user_id_str] = {'name': name, 'rated_identifiers': [
        ], 'phone': None, 'orders': [], 'feedback_ratings': {}}
        save_users(USERS)
    ...
    if identifier in USERS[user_id_str]['rated_identifiers']:
        ...
        logger.info(
            f"❗ Qayta baholash urunishi: user_id={user_id}, identifier={identifier}")
        return
    ...
    save_users(USERS)
    logger.info(
        f"✅ Baho qayd etildi: user_id={user_id}, identifier={identifier}, rating={rating}")
    ...
    try:
        ...
        logger.error(
            f"❌ Guruhga xabar yuborishda xato: user_id={user_id}, xato={e}")
    ...
    try:
        ...
        logger.error(
            f"❌ Tugmalarni o‘chirishda xato: user_id={user_id}, xato={e}")
    ...
    if rating <= 3:
        ...
        context.user_data['waiting_for_feedback'] = {
            'identifier': identifier, 'name': name, 'is_help_request': is_help_request, 'feedback_count': 0, 'feedback_messages': []}
...
async def rating_feedback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.debug(
        f"Feedback qo'shildi: user_id={user_id}, count={feedback_data['feedback_count']}")
...
async def command_in_topic_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message.text.startswith("/bekor"):
        ...
        try:
            ...
            logger.info(
                f"✅ Buyurtma bekor qilindi: user_id={user_id}, order_id={order_id}")
        except Exception as e:
            logger.error(
                f"❌ Foydalanuvchiga bekor qilish xabari yuborilmadi: user_id={user_id}, xato={e}")
        ...
        try:
            ...
            logger.info(
                f"✅ Buyurtma bajarildi: user_id={user_id}, order_id={order_id}")
        except Exception as e:
            logger.error(
                f"❌ Foydalanuvchiga bajarildi xabari yuborilmadi: user_id={user_id}, xato={e}")
            await message.reply_text("❌ Foydalanuvchiga xabar yuborishda xato yuz berdi.")
    ...
    try:
        ...
        logger.info(
            f"✅ Baholash so‘rovi yuborildi: user_id={user_id}, order_id={order_id}")
    except Exception as e:
        logger.error(
            f"❌ Baholash so‘rovi yuborishda xato: user_id={user_id}, xato={e}")
    ...
    try:
        ...
        logger.info(
            f"✅ context.bot_data tozalandi: user_id={user_id}, thread_id={thread_id}")
    except Exception as e:
...
async def user_file_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == 'payment':
        logger.info(
            f"✅ To‘lov cheki sifatida qayta ishlash: user_id={user_id}, file_type={file_type}")
        order_id = context.user_data.get('order_id')
        if not order_id:
        ...
        if not payment_info:
            ...
            logger.error(
                f"❌ payment_info topilmadi: user_id={user_id}, order_id={order_id}")
            context.user_data.pop('mode', None)
            return

        user_id_str = str(user_id)
        order = next(
            (o for o in USERS[user_id_str]['orders'] if o['order_id'] == order_id), None)
        if not order:
            await update.message.reply_text("❌ Buyurtma topilmadi.")
            logger.error(
                f"❌ Buyurtma topilmadi: user_id={user_id}, order_id={order_id}")
            context.user_data.pop('mode', None)
            return
        ...
        markup = InlineKeyboardMarkup([
            [InlineKeyboardButton(
                "✅ Mijozga tasdiq yuborish", callback_data=f"confirm_payment_{order_id}")]
        ])
        ...
        try:
            ...
            logger.info(
                f"✅ To‘lov cheki guruhga yuborildi: user_id={user_id}, order_id={order_id}")
        except Exception as e:
            await update.message.reply_text("❌ Chek yuborishda xato yuz berdi. Qayta urinib ko‘ring.")
            logger.error(
                f"❌ Chek guruhga yuborishda xato: user_id={user_id}, xato={e}")
    ...
    logger.info(
        f"✅ Fayl chat sifatida qayta ishlash: user_id={user_id}, file_type={file_type}")
    user_data = context.bot_data.get(f"user_{user_id}")
    ...
    try:
        ...
        await update.message.reply_text("✅ Faylingiz operatorga yuborildi.")
        logger.info(
            f"✅ Fayl chatga yuborildi: user_id={user_id}, thread_id={thread_id}")
    except Exception as e:
        await update.message.reply_text("❌ Fayl yuborishda xato yuz berdi. Qayta urinib ko‘ring.")
        logger.error(
            f"❌ Fayl chatga yuborishda xato: user_id={user_id}, thread_id={thread_id}, xato={e}")
...
async def show_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_page > 0:
        buttons.append(InlineKeyboardButton(
            "⬅️ Oldingi", callback_data=f"history_prev_{current_page-1}"))
    if current_page < total_pages - 1:
        buttons.append(InlineKeyboardButton(
            "Keyingi ➡️", callback_data=f"history_next_{current_page+1}"))
...
async def pay_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not order_id or not user_id:
        logger.error(
            f"❌ order_id yoki user_id topilmadi: thread_id={thread_id}")
    ...
    try:
        service = next((s for s in get_services()
                       if s['id'] == order['service_id']), None)
        if not service:
            raise ValueError("Xizmat topilmadi.")
        amount = service['price']
    except Exception as e:
        logger.error(
            f"❌ Xizmat narxi topilmadi: order_id={order_id}, xato={e}")
    ...
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("📤 Chekni yuborish",
    ...
    try:
        ...
        logger.error(
            f"❌ Grafik invoys yuborishda xato: user_id={user_id}, xato={e}")
        await context.bot.send_message(
            chat_id=user_id,
            text=invoice_text,
            parse_mode=ParseMode.HTML,
    ...
    save_bot_data(context)
    logger.info(
        f"✅ Invoys yuborildi: user_id={user_id}, order_id={order_id}, amount={amount}")
...
async def send_receipt_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(
        f"✅ To‘lov cheki so‘raldi: user_id={query.from_user.id}, order_id={order_id}")
...
async def receipt_file_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    order = next((o for o in USERS[user_id]['orders']
    ...
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Mijozga tasdiq yuborish",
    ...
    try:
        ...
        save_bot_data(context)
        logger.info(
            f"✅ Chek guruhga yuborildi: user_id={user_id}, order_id={order_id}")
...
async def confirm_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(
        f"✅ To‘lov tasdiqlandi: user_id={user_id}, order_id={order_id}")
...
async def confirm_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    confirmation_time = datetime.now(pytz.timezone(
    ...
    try:
        receipt_image = create_receipt_image(
            order, payment_info['amount'], confirmation_time)
    except Exception as e:
        logger.error(
            f"❌ Chek rasmi yaratishda xato: user_id={user_id}, xato={e}")
        receipt_image = None
    ...
    try:
        ...
        logger.error(
            f"❌ Mijozga xabar yuborishda xato: user_id={user_id}, xato={e}")
    ...
    try:
        ...
        logger.error(
            f"❌ Operator xabarini yangilashda xato: order_id={order_id}, xato={e}")

    logger.info(
        f"✅ To‘lov tasdiqlandi va chek yuborildi: user_id={user_id}, order_id={order_id}")
...
async def admin_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.debug(
        f"Admin tekshiruvi boshlandi: user_id={user_id}, ADMIN_ID={ADMIN_ID}, secret_key={secret_key}")
    if user_id != ADMIN_ID:
    ...
    if context.user_data.get("admin_secret") != secret_key:
        markup = InlineKeyboardMarkup(
            [[InlineKeyboardButton("🔑 Kirish", callback_data=f"verify_secret_{secret_key}")]])
        await update.message.reply_text(
            "🔐 Admin paneliga kirish uchun tasdiqlang:",
            reply_markup=markup
        )
        logger.info(
            f"✅ Admin autentifikatsiyasi so‘raldi: user_id={user_id}, secret_key={secret_key}")
        return
...
async def verify_inline_secret(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.debug(
        f"Maxfiy kalit tekshiruvi: received_key={received_key}, secret_key={secret_key}, callback_data={query.data}")
    if received_key == secret_key:
        ...
        logger.info(
            f"✅ Admin maxfiy kaliti tasdiqlandi: user_id={query.from_user.id}")
    else:
        await query.edit_message_text("❌ Noto‘g‘ri maxfiy kalit! Iltimos, qayta urining.")
        logger.error(
            f"❌ Noto‘g‘ri maxfiy kalit urinishi: user_id={query.from_user.id}, received={received_key}, expected={secret_key}")
    return ConversationHandler.END
...
async def list_services_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for s in services:
        ...
        markup = InlineKeyboardMarkup([
            [
                InlineKeyboardButton(
                    "✏️ Tahrirlash", callback_data=f"edit_service_{s['id']}"),
                InlineKeyboardButton(
                    "🗑 O‘chirish", callback_data=f"delete_service_{s['id']}")
            ]
        ])
        messages.append(query.message.reply_text(
    ...
    try:
        ...
        if not service:
            await update.message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton(
                        "⬅️ Xizmatlar menyusiga", callback_data="admin_services")]
                ])
            )
            logger.error(
                f"❌ Xizmat topilmadi: user_id={update.effective_user.id}, service_id={service_id}")
            return EDIT_SERVICE_ID
        context.user_data['edit_service_id'] = service_id
        return await select_edit_field(update, context)
    except ValueError:
...
async def select_edit_field(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    markup = InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("💳 To‘lov usullari",
                                 callback_data="edit_payments"),
    ...
    logger.info(
        f"✅ Tahrirlash maydonlari ko‘rsatildi: user_id={update.effective_user.id}, service_id={service_id}")
...
async def edit_service_field(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(
                "⬅️ Orqaga", callback_data="select_edit_field")]
        ])
    )
    logger.info(
        f"✅ Tahrirlash uchun maydon tanlandi: user_id={query.from_user.id}, field={field}")
    return {
...
async def edit_service_payments(update: Update, context: ContextTypes.DEFAULT_TYPE):
    payments = [p.strip()
                for p in update.message.text.strip().split(',') if p.strip()]
    if not payments:
...
async def save_edited_field(update: Update, context: ContextTypes.DEFAULT_TYPE, field, value):
    ...
    logger.info(
        f"✅ Xizmat tahrirlandi: user_id={update.effective_user.id}, service_id={service_id}, field={field}")
    return EDIT_SERVICE_FIELD
...
async def delete_service_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data
    # Debag qo‘shish
    logger.debug(f"Delete service handler: callback_data={data}")
    if data.startswith("delete_service_") and data != "delete_service":
        ...
        markup = InlineKeyboardMarkup([
            [
                InlineKeyboardButton(
                    "✅ Tasdiqlash", callback_data=f"confirm_delete_{service_id}"),
                InlineKeyboardButton(
                    "❌ Bekor qilish", callback_data="admin_services")
            ]
        ])
        await query.edit_message_text(
        ...
        logger.info(
            f"✅ Xizmat o‘chirish tasdiqlash so‘raldi: user_id={query.from_user.id}, service_id={service_id}")
...
async def confirm_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await update.message.reply_text(text, parse_mode=ParseMode.HTML, reply_markup=markup)
    logger.info(
        f"✅ Xizmat o‘chirildi: user_id={update.effective_user.id}, service_id={service_id}")
    return ConversationHandler.END
...
async def search_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not matches:
        ...
        logger.info(
            f"ℹ Qidiruv natijasiz: user_id={update.effective_user.id}, query={query_text}")
        return SEARCH_SERVICE
    ...
    logger.info(
        f"✅ Qidiruv natijalari ko‘rsatildi: user_id={update.effective_user.id}, query={query_text}, count={len(matches)}")
    return SEARCH_SERVICE
...
async def group_by_category_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    services = get_services()
    categories = sorted(set(s.get('category', 'Belgilanmagan')
                        for s in services))
    if not categories:
        await query.edit_message_text(
    ...
    for i in range(0, len(categories), 2):
        row = [InlineKeyboardButton(
            categories[i], callback_data=f"view_category_{categories[i]}")]
        if i + 1 < len(categories):
            row.append(InlineKeyboardButton(
                categories[i + 1], callback_data=f"view_category_{categories[i + 1]}"))
        buttons.append(row)
    buttons.append([InlineKeyboardButton(
        "⬅️ Xizmatlar menyusiga", callback_data="admin_services")])
    ...
    logger.info(
        f"✅ Toifalar ro‘yxati ko‘rsatildi: user_id={query.from_user.id}, count={len(categories)}")
...
async def view_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    services = get_services()
    category_services = [s for s in services if s.get(
        'category', 'Belgilanmagan') == category]
    ...
    if not category_services:
        ...
        logger.info(
            f"ℹ Toifa bo‘sh: user_id={query.from_user.id}, category={category}")
        return
    ...
    logger.info(
        f"✅ Toifa xizmatlari ko‘rsatildi: user_id={query.from_user.id}, category={category}, count={len(category_services)}")
...
async def toggle_service_visibility_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(
        f"✅ Faollik o‘zgartirish boshlandi: user_id={query.from_user.id}")
...
async def toggle_visibility(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.info(
            f"✅ Xizmat faolligi o‘zgartirildi: user_id={update.effective_user.id}, service_id={service_id}, status={status}")
        context.user_data.clear()
        return ConversationHandler.END
    except ValueError:
        await update.message.reply_text(
...
async def admin_payments_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    context.user_data.clear()
    payments = {k: v for k, v in context.bot_data.items()
    ...
    for key, value in payments.items():
        ...
        if value['status'] == 'awaiting_confirmation':
            buttons.append([InlineKeyboardButton(
                f"✅ Tasdiqlash #{order_id}", callback_data=f"confirm_payment_{order_id}")])
    markup = InlineKeyboardMarkup(get_admin_payments_buttons(
    ) + buttons if buttons else get_admin_payments_buttons())
    await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=markup)
...
async def admin_users_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if page > 0:
        buttons.append(InlineKeyboardButton(
            "⬅️ Oldingi", callback_data=f"users_page_{page-1}"))
    if (page + 1) * per_page < len(USERS):
        buttons.append(InlineKeyboardButton(
            "Keyingi ➡️", callback_data=f"users_page_{page+1}"))
...
async def admin_stats_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    context.user_data.clear()
    orders = [order for user in USERS.values()
...
async def handle_announce_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    error_text = f" {errors} ta xatolik yuz berdi." if success_count < len(
        results) else ""

    await update.message.reply_text(
        f"✅ E’lon {success_count} ta foydalanuvchiga yuborildi!{error_text}",
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton(
                "🔙 Admin panel", callback_data="admin_main")]]
        )
    )
...
async def admin_main_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    markup = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "📦 Xizmatlar", callback_data="admin_services"),
            InlineKeyboardButton("📋 Buyurtmalar", callback_data="admin_orders")
        ],
        [
            InlineKeyboardButton(
                "💸 To‘lovlar", callback_data="admin_payments"),
            InlineKeyboardButton("👥 Foydalanuvchilar",
                                 callback_data="admin_users")
        ],
        [
            InlineKeyboardButton("📊 Statistika", callback_data="admin_stats"),
            InlineKeyboardButton(
                "⚙️ Sozlamalar", callback_data="admin_settings")
        ]
    ])
...
def main():
    ...
    conv_handler = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex(r"^#XIZMAT#\d+$"),
                           trigger_inline_handler),
            CommandHandler("start", start),
            CommandHandler("admin", admin_main_menu),
            CallbackQueryHandler(add_service_handler,
                                 pattern=r"^add_service$"),
            CallbackQueryHandler(edit_service_handler,
                                 pattern=r"^edit_service(_\d+)?$"),
            CallbackQueryHandler(delete_service_handler,
                                 pattern=r"^delete_service(_\d+)?$"),
            CallbackQueryHandler(search_service_handler,
                                 pattern=r"^search_service$"),
            CallbackQueryHandler(
                toggle_service_visibility_handler, pattern=r"^toggle_service_visibility$"),
            CallbackQueryHandler(group_by_category_handler,
                                 pattern=r"^group_by_category$"),
            CallbackQueryHandler(view_category, pattern=r"^view_category_"),
            CallbackQueryHandler(
                select_edit_field, pattern=r"^select_edit_field$"),
            CallbackQueryHandler(admin_services_handler,
                                 pattern=r"^admin_services$"),
            CallbackQueryHandler(admin_orders_handler,
                                 pattern=r"^admin_orders$"),
            CallbackQueryHandler(admin_payments_handler,
                                 pattern=r"^admin_payments$"),
            CallbackQueryHandler(admin_users_handler,
                                 pattern=r"^admin_users$"),
            CallbackQueryHandler(admin_stats_handler,
                                 pattern=r"^admin_stats$"),
            CallbackQueryHandler(admin_settings_handler,
                                 pattern=r"^admin_settings$"),
            CallbackQueryHandler(admin_main_handler, pattern=r"^admin_main$"),
            CallbackQueryHandler(rating_callback_handler,
                                 pattern=r"^rate_"),  # Qo‘shildi
            CallbackQueryHandler(settings_admin_handler,
                                 pattern=r"^settings_admin$"),
            CallbackQueryHandler(admin_settings_handler,
                                 pattern=r"^admin_settings$"),
        ],
        states={
            ...
            WAIT_CONTACT_METHOD: [
                CallbackQueryHandler(
                    contact_method_handler, pattern=r"^contact_"),
            ...
            EDIT_SERVICE_FIELD: [
                CallbackQueryHandler(
                    edit_service_field, pattern=r"^edit_(name|price|payments|image|category)$"),
                CallbackQueryHandler(
                    select_edit_field, pattern=r"^select_edit_field$")
            ],
            ...
            DELETE_SERVICE_ID: [
                MessageHandler(filters.TEXT & ~filters.COMMAND,
                               confirm_delete),
                CallbackQueryHandler(
                    confirm_delete, pattern=r"^confirm_delete_\d+$")
            ],
            ...
            # Qo‘shildi
            "rating_feedback": [MessageHandler(filters.TEXT & ~filters.COMMAND & filters.USER, rating_feedback_handler)]
        },
        fallbacks=[
            CallbackQueryHandler(admin_settings_handler,
                                 pattern=r"^admin_settings$"),
            MessageHandler(filters.TEXT & ~filters.COMMAND &
                           filters.USER, fallback_handler),
            CallbackQueryHandler(verify_inline_secret,
                                 pattern=r"^verify_secret_"),
            CallbackQueryHandler(restart_handler, pattern=r"^restart$"),
            CallbackQueryHandler(continue_handler, pattern=r"^continue$"),
            CallbackQueryHandler(help_request_handler,
                                 pattern=r"^help_request$"),
            CallbackQueryHandler(admin_services_handler,
                                 pattern=r"^admin_services$"),
            CallbackQueryHandler(admin_main_handler, pattern=r"^admin_main$")
        ],
    ...
    app.add_handler(CallbackQueryHandler(
        roziman_handler, pattern=r"^confirm_service$"))
    app.add_handler(CallbackQueryHandler(
        contact_method_handler, pattern=r"^contact_"))
    app.add_handler(MessageHandler(
        filters.Regex(r"^/pay"), pay_command_handler))
    app.add_handler(MessageHandler(filters.Regex(
        r"^/(bajarildi|bekor)"), command_in_topic_handler))
    app.add_handler(MessageHandler(
        filters.ALL & filters.Chat(GROUP_ID), relay_from_group))
    app.add_handler(CallbackQueryHandler(
        group_order_buttons, pattern=r"^group_(accept|cancel)_"))
    app.add_handler(CallbackQueryHandler(
        rating_callback_handler, pattern=r"^rate_"))
    app.add_handler(CallbackQueryHandler(
        help_request_handler, pattern=r"^help_request$"))
    app.add_handler(CallbackQueryHandler(
        show_history, pattern=r"^show_history$"))
    app.add_handler(CallbackQueryHandler(
        history_pagination_handler, pattern=r"^history_(prev|next)_"))
    app.add_handler(CallbackQueryHandler(
        send_receipt_handler, pattern=r"^send_receipt_"))
    app.add_handler(MessageHandler(
        filters.PHOTO | filters.Document.ALL, user_file_handler))
    app.add_handler(MessageHandler(
        filters.PHOTO | filters.Document.ALL & filters.USER, user_file_handler))
    app.add_handler(CallbackQueryHandler(
        confirm_payment_handler, pattern=r"^confirm_payment_"))
    app.add_handler(MessageHandler(
        filters.ALL & filters.USER, universal_router))
    app.add_handler(MessageHandler(
        filters.TEXT & filters.USER, rating_feedback_handler))
    app.add_handler(CallbackQueryHandler(
        accept_help_button_handler, pattern=r"^accept_help_"))
    app.add_handler(CallbackQueryHandler(
        restart_handler, pattern=r"^restart$"))
    app.add_handler(CallbackQueryHandler(
        continue_handler, pattern=r"^continue$"))
    app.add_handler(MessageHandler(
        filters.TEXT & filters.USER, fallback_handler))
    app.add_error_handler(error_handler)
    app.add_handler(CallbackQueryHandler(
        bonus_services_handler, pattern="^bonus_services$"))
    app.add_handler(CallbackQueryHandler(
        roziman_handler, pattern="^confirm_service$"))
    # /start 000xxx formatidagi so‘rovlarni ushlab qolish
    app.add_handler(CommandHandler("start", handle_start_with_service,
                    filters=filters.Regex(r"^/start\s+000\d{3}$")))

    application = Application.builder().token(os.environ["BOT_TOKEN"]).build()
    app.add_handler(CommandHandler("start", handle_start_with_service,
                    filters=filters.Regex(r"^/start 000\d{3}$")))
